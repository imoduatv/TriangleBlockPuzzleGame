		...
		public class FieldWithRemoteSettingsKey
		{
			...
			public object SetValueRecursive(object val, object target, string path)
			{
				...
				else
				{
					if (property == null)
					{
						throw new InvalidOperationException($"Member '{text}' on target {target} is neither a field nor property");
					}
					...
					if (indexParameters.GetLength(0) == 1 && indexParameters[0].ParameterType == typeof(int))
					{
						...
						if (array[0] != null && int.TryParse(array[0], out int result))
						{
							index = new object[1]
							{
								result
				...
				if (obj != null)
				{
					...
					else
					{
						...
						{
							throw new InvalidOperationException($"Property '{text}' on target {target} is readonly");
						}
